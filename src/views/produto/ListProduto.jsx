...
export default function ListProduto() {
  ...
  function carregarLista() {
    ...
    axios.get('http://localhost:8080/api/categoriaproduto').then((response) => {
      ...
      response.data.map((c) => dropDownCategorias.push({ text: c.descricao, value: c.id }));
  ...
  async function filtrarProdutos(codigoParam, tituloParam, idCategoriaParam) {
    ...
    await axios.post('http://localhost:8080/api/produto/filtrar', formData).then((response) => {
        setLista(response.data);
    });
  }
            ...
            {menuFiltro ? (
              <Segment>
                <Form className="form-filtros">
                  <Form.Input icon="search" value={codigo} onChange={(e) => handleChangeCodigo(e.target.value)} label="Código do Produto" placeholder="Filtrar por Código do Produto" labelPosition="left" width={4} />
                  <Form.Group widths="equal">
                    <Form.Input icon="search" value={titulo} onChange={(e) => handleChangeTitulo(e.target.value)} label="Título" placeholder="Filtrar por título" labelPosition="left" />
                    <Form.Select
                ...
                {lista.map((produto) => (
                  <Table.Row key={produto.id}>
                    ...
                    <Table.Cell textAlign="center">
                      ...
                      &nbsp;
                      <Button inverted circular color="red" title="Clique aqui para remover este produto" icon onClick={(e) => confirmaRemover(produto.id)}>
                        <Icon name="trash" />
                      </Button>
                    </Table.Cell>
                  </Table.Row>
